...
```java
private static final String DB\\\_URL      = "jdbc:mysql://localhost:3306/live\\\_auction\\\_db?useSSL=false\\\&serverTimezone=UTC";
private static final String DB\\\_USER     = "root";
private static final String DB\\\_PASSWORD = "YOUR\\\_MYSQL\\\_PASSWORD";  // ← change this
```
...
|`Access denied for user 'root'@'localhost'`|Check DB\_PASSWORD in DBConnection.java|
...
```sql
users    (id, name, email, password, role, created\\\_at)
items    (id, seller\\\_id→users, name, description, start\\\_price, image\\\_path, created\\\_at)
auctions (id, item\\\_id→items, start\\\_time, end\\\_time, status, winner\\\_id→users, winning\\\_bid, created\\\_at)
bids     (id, auction\\\_id→auctions, bidder\\\_id→users, bid\\\_amount, bid\\\_time)
```
...
* `GET /buyer?action=dashboard` → `buyer\\\_dashboard.jsp`
